    ...
    function ($, _, Hypr, Backbone, api, CustomerModels, AddressModels, PaymentMethods, HyprLiveContext) {
        ...
        FulfillmentContact = CheckoutStep.extend({
            ...
            initialize: function () {
                ...
                this.set('address.countryCode',"US");
                // this.set('address.addressType','Nan');
            },
            calculateStepStatus: function () {
        ...
        BillingInfo = CheckoutStep.extend({
            ...
            validateSavedPaymentMethodId: function (value, attr, computedState) {
                if (this.get('usingSavedCard')) {
                    ...
                    if (!isValid && isValid !==undefined ) return Hypr.getLabel('selectASavedCard');
                }
            },
            ...
            resetAddressDefaults: function () {
				...
				if(addressDefaults.countryCode === ""){
                    addressDefaults.countryCode = "US";
                }
                ...
                if(purchaseOrderEnabled) {
                    if(currentPurchaseOrder.selected && contacts.length > 0) {
                       /* var foundBillingContact = contacts.models.find(function(item){
                         ...
                         var foundBillingContact;
                         
                         $.each(contacts.models,function(i,v){
                            //console.log(v); 
                            if(v.get('isPrimaryBillingContact')){
                                foundBillingContact = v;    
                            }
                         });
            ...
            submit: function () {
                ...
                if (this.nonStoreCreditTotal() > 0 && val) {
                    ...
                    if (error.items.length > 0) {
                       /* if($('#mz-payment-purchase-order-number').val()){
                          $('html,body').animate({scrollTop: $('#copyshipping').offset().top});
                        }*/
                        /*if(this.get('paymentType').toLowerCase() === "purchaseorder" && !this.get('purchaseOrder.isEnabled') ){
                           $('.paymentTerm').show(); 
                        }*/
             ...
             /*   if(this.get('paymentType').toLowerCase() === "purchaseorder" && this.get('purchaseOrder.isEnabled')) {
                    this.get('purchaseOrder').inflateCustomFields();
                }else if(this.get('paymentType').toLowerCase() === "purchaseorder" && !this.get('purchaseOrder.isEnabled') && order.get('customer.purchaseOrder').creditLimit < 1 ){
                     $('.paymentTerm').show();
                     return false;
                }*/
        ...
        var CheckoutPage = Backbone.MozuModel.extend({
            ...
            addCoupon: function () {
                ...
                return this.apiAddCoupon(this.get('couponCode')).then(function () {
                    ...
                    if (me.get('suggestedDiscounts').length && autoAddSuggestedDiscount){
                        ...
                        $(document).find('.autoaddredirect').show();                        
                        setTimeout(function(){window.location =  (HyprLiveContext.locals.siteContext.siteSubdirectory||'') + "/cart";},5000);  
            ...
            onCheckoutError: function (error) {
                ...
                //Adding custome message for Inventory Out Message
                if(error.message){
                    if (error.message.indexOf("does not have enough stock to create this reservation and backorders are not enabled. Reservations are not allowed at this time.") > 0) {
                        var productCode = error.message.substring(error.message.lastIndexOf("Validation Error: Product ") + 26, error.message.lastIndexOf("does not have enough stock to create this reservation and backorders are not enabled. Reservations are not allowed at this time.") - 1);
                        $.each(order.apiModel.data.items, function(key, val) {
                            if (val.product.productCode == productCode) {
                                error.message = 'Unfortunately, <p style="display: inline-block;font-weight: bold;margin: 0px;">' + val.product.name + '</p> is not available at this time and so we can’t place your order. Please select another item or remove it from your cart before trying again. If you need help, you can call our friendly Consumer Affairs representatives at 1-800-522-3267 (Mon-Fri, 7am to 5pm PT). Thanks and sorry!';
                            }
                        });
                    }
                }
            ...
            submit: function () {
                ...
                if (nonStoreCreditTotal > 0 && this.validate()) {
                    if(order.get('billingInfo').activePayments()[0].paymentType=== "PurchaseOrder"){
                        if(!(order.get('billingInfo.purchaseOrder.customFields').models[0].get('value'))){
