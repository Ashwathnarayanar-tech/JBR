define(['modules/jquery-mozu', 'underscore', 'modules/backbone-mozu', 'hyprlive', 'hyprlivecontext', 'modules/models-address'], function ($, _, Backbone, Hypr, HyprLiveContext, Address) {
    ...
    var PaymentMethod = Backbone.MozuModel.extend({
        present: function (value, attr) {
            ...
            if (!value) {
                if(attr ==="purchaseOrder.pOCustomField-shipdate" && $('#datePicker').datepicker('getDate')){
                    return false;
                }else if(attr == "purchaseOrder.purchaseOrderNumber"){
                    $('.reqnumbers').css("color","#b94a48");
                    $('html,body').animate({scrollTop: $('.paymenttypes-choosing').offset().top});
                    return this.validation[attr.split('.').pop()].msg || Hypr.getLabel('genericRequired'); 
                }else{
                     $('.reqnumbers').css("color","#8c8c8c");
                      return this.validation[attr.split('.').pop()].msg || Hypr.getLabel('genericRequired'); 
                } 
               
            }else if(attr == "purchaseOrder.purchaseOrderNumber" && value.length<3){
    ...
    var CreditCard = PaymentMethod.extend({
        ...
        expirationDateInPast: function (value, attr, computedState) {
            ...
            if (!isValid) return Hypr.getThemeSetting('expires');
        },
            ...
            siteSettingsCustomFields.forEach(function(field) {
                if(field.isEnabled) {
                    ...
                    if(this.get("pOCustomField-"+field.code) !== undefined && this.get("pOCustomField-"+field.code) !==""){
                        ...
                        value = $('#datePicker').datepicker({ dateFormat: 'MM,dd,yyyy' }).val();
                    }else if(field.code === 'profileid' || field.code ==='cardnumber'){
                        value = "";
                    }
